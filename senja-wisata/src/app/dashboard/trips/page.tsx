...
import { Calendar, Clock, MapPin, Users, ChevronDown, ChevronUp, Star, Download, XCircle, AlertTriangle, CheckCircle, CreditCard, Loader2 } from "lucide-react";
...
export default function TripsPage() {
    ...
    return (
        <div className="space-y-6">
            {/* Toast */}
            {toast && (
                <div className={`fixed top-6 right-6 z-50 px-5 py-3 rounded-xl shadow-xl font-semibold text-sm flex items-center gap-2 transition-all ${toast.type === "success" ? "bg-emerald-500 text-white" :
            ...
            <div className="flex flex-wrap gap-1 border-b border-slate-100 pb-1">
                {([
                    ...
                    <button
                        key={key}
                        onClick={() => setTab(key)}
                        className={`px-4 py-2 text-sm font-semibold rounded-t-lg transition-all ${tab === key ? "text-accent border-b-2 border-accent" : "text-slate-400 hover:text-primary"
                            }`}
                    >
                        {label}
                                    ...
                                    <div className="flex flex-wrap items-start gap-3 justify-between mb-3">
                                        <div>
                                            <div className="flex items-center gap-2 mb-2">
                                                ...
                                                <span className={`badge text-xs ${paymentBadge[b.payment_status] || "bg-slate-100 text-slate-500"}`}>
                                                    <CreditCard size={10} className="mr-1" />
                                                    {paymentLabel[b.payment_status] || b.payment_status}
                                                </span>
                                            </div>
                                            <h3 className="font-bold text-primary text-lg">{b.tour_title || `Tour #${b.tour_id}`}</h3>
                                            {b.tour_location && (
                                                <div className="flex items-center gap-1 text-slate-400 text-sm">
                                                    <MapPin size={13} /> {b.tour_location}
                                                </div>
                                            )}
                                        </div>
                                        <div className="text-right">
                                            <div className="font-bold text-accent text-lg">{formatPrice(b.total_price)}</div>
                                            <div className="text-xs text-slate-400 font-mono">{b.booking_code}</div>
                                        </div>
                                    </div>

                                    <div className="flex flex-wrap gap-4 text-sm text-slate-500 mb-4">
                                        <span className="flex items-center gap-1"><Calendar size={13} /> {b.tour_date}</span>
                                        <span className="flex items-center gap-1"><Users size={13} /> {b.guests} Orang</span>
                                        {b.adults > 0 && <span className="text-xs text-slate-400">{b.adults} Dewasa{b.children > 0 ? `, ${b.children} Anak` : ""}</span>}
                                    </div>
                                        ...
                                        <div className="sm:col-span-2">
                                            <div className="rounded-xl bg-blue-50 border border-blue-100 p-4">
                                                ...
                                                <div className="flex items-center justify-between gap-4">
                                                    <div className="font-mono text-lg font-bold text-primary tracking-widest">{b.va_number}</div>
                                                    <button
